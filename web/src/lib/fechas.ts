...
/**
 * Tiempo relativo desde un timestamp ISO (última señal AirTag).
 * Ej: "hace 3 min", "hace 2 h", "hace 1 d".
 */
export function haceCuanto(iso: string | undefined | null): string {
  if (!iso?.trim()) return "Sin señal";
  const t = new Date(iso.trim()).getTime();
  if (Number.isNaN(t)) return "Sin señal";
